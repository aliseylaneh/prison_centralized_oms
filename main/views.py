...
def review_request(request):
    if request.method == 'POST':
        ...
        request_n.number = get_request_id(prison.id, request_n)
        request_n.save()
...
def declined_request(request):
    if request.user.groups.all()[0].name == 'ceo':
        requests_r = Request.objects.filter(request_status=Status.ceo_dreview).order_by('-created_date')
    elif request.user.groups.all()[0].name == 'commercial_manager':
...
def get_request_id(prison, request_n):
    return f"{date2jalali(request_n.created_date).strftime('%Y')}{date2jalali(request_n.created_date).strftime('%m')}{prison}{request_n.id}"
